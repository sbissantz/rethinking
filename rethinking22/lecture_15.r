...
# (2) Generative model - just dyad effects
# Note: Here we exclude the condounding variables
...
vcol <- "#DE536B"
plot(sng , layout=lx , vertex.size=8 , edge.arrow.size=0.75 , edge.width=2 ,
     edge.curved=0.35 , vertex.color=vcol , edge.color=grau() , asp=0.9 ,
     margin = -0.05 , vertex.label=NA )
...
file <- file.path(path, "stan", "15", "1_cnt.stan")
...
fit$cmdstan_diagnose() # Divergences!
fit$diagnostic_summary()

# Stan
#
# Non-centered parametrization
# Note; When I use the poisson_log; I get differences
# in the approximation of alpha by 0.2?!
path <- "~/projects/rethinking22"
file <- file.path(path, "stan", "15", "1_nct.stan")
mdl <- cmdstanr::cmdstan_model(file, pedantic=TRUE)
fit <- mdl$sample(data=sim_data)
fit$print(max_rows=200)
...
# A little different approximation of the posterior
post <- fit$draws(format = "data.frame") 
T <- fit$draws("T", format = "matrix") 
T_est <- apply(T, 2, mean)
dim(T_est) <- c(300, 2)
...
# Note: aboves codes is unchanged – and only from the lecture script
...
path <- "~/projects/rethinking22"
file <- file.path(path, "stan", "15", "3_nct.stan")
mdl <- cmdstanr::cmdstan_model(file, pedantic=TRUE)
...
# Contrast
dens(post2$bW[,1] - post2$bW[,2], lwd=4 , col=3 , xlab="Wealth contrast" )
